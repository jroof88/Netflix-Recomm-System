...
def case_modification(val): #CaseMod function
    newVal = pow(val, 2.5)
    return newVal

def ratingAvg(arr): #Average rating function to get the average rating
    numerator = 0.0
    denominator = 0.0
    for rating in arr:
        if(rating != 0):
...
def pearson_correlationW(train, test): #Pearson Weight function
...
def runPearson(num): #Main driver for Pearson
